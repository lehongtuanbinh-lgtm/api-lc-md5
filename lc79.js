...
// 12. NEW: ULTIMATE VIP BRIDGE BREAKER - Bẻ cầu chuyên sâu
function algoL_UltimateBridgeBreaker(history) {
    ...
    const lastRun = runs[runs.length - 1];
    if (lastRun.len < 4) return null; // Chỉ xem xét bẻ khi cầu dài >= 4
    
    // Tính toán độ dài chuỗi trung bình và độ lệch chuẩn của các chuỗi cùng loại
    ...
    // Bẻ ngay lập tức nếu chuỗi hiện tại vượt qua 95th percentile (mean + 2*std)
...
// --- NEW: DỰ ĐOÁN VỊ KẾT QUẢ 3 XÚC XẮC ---
function predictDiceValues(history, predictedTx) {
    ...
    // Tìm bộ 3 xúc xắc khớp với targetSum, ưu tiên các mặt có tần suất cao (penalty thấp)
    ...
    // Xáo trộn nhẹ để kết quả tự nhiên hơn
...
const ALL_ALGS = [
    ...
    { id: 'l_ultimate_bridge_breaker', fn: algoL_UltimateBridgeBreaker } // THUẬT TOÁN BẺ CẦU MỚI
];
...
function getComplexPattern(history) {
    ...
    // Ghép cả Pattern cũ và VIP Pattern mới
    ...
    if (vipPat && vipPat.length > 0) {
        // Lấy tên tiếng việt để trả về API
...
app.get("/api/taixiumd5/lc79", async () => {
    ...
    // Tích hợp hệ thống dự đoán Vị 3 xúc xắc
    ...
    if (!lastResult || !currentPrediction) {
        return {
            ...
            phien_hien_tai: null,
            du_doan: "chưa có",
            du_doan_xuc_xac: null, // Mảng dự đoán vị
            do_tin_cay: "0%"
        };
    }
    ...
    return {
        ...
        du_doan_xuc_xac: dicePrediction, // [d1, d2, d3]
...
app.get("/", async () => { 
    return {
        ...
        version: "4.5", // Up version
        algorithms: ALL_ALGS.length,
        pattern_recognition: "siêu cấp (36+ mẫu phức tạp & dự đoán Vị)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
`;
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro - Ultimate VIP Pattern Master đã khởi động!");
    ...
    console.log("\n🎯 Nhận diện 36+ mẫu cầu phức tạp (Đã nâng cấp BẺ CẦU VIP & VỊ XÚC XẮC):");
    console.log("   • Bẻ cầu chuẩn xác qua độ lệch chuẩn và động lượng (Momentum).");
    console.log("   • Dự đoán vị 3 xúc xắc dựa trên Total-Sum Distribution.");
    console.log("   • Cầu Bệt, Rồng, Đảo 1-1, Nhảy Cóc, 2-2, 3-3, 4-4, 5-5");
    console.log("   • Cầu 2-1-2, 1-2-1, 3-2-3, 4-2-4, 1-2-2-1, 1-2-3, 3-2-1");
    console.log("   • Dây Gãy MD5, Golden Ratio, Wave Pattern, Đảo Xu Hướng");
};
